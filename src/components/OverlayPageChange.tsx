...
export default function OverlayPageChange({ active, animSpeed }: OpcProps) {
  const animLeft = {
    active: { x: '0%', transition: { duration: `${animSpeed}` } },
    inactive: { x: '-100%', transition: { duration: 0, ease: 'linear' } }
  }
  const animRight = {
    active: { x: '0%', rotate: '180deg', transition: { duration: `${animSpeed}` } },
    inactive: { x: '100%', rotate: '180deg', transition: { duration: 0, ease: 'linear' } },
  }
  return (
    <>
      <MotionBox
